# =====================================================
#  ULTIMATE RUNTIMES INSTALLER - PAINEL PROFISSIONAL
...
function Install-Runtime {
    ...
    try {
        ...
        Write-Host "   ⚙️ Instalando (aguarde)..." -ForegroundColor Gray
        ...
        if ($process.ExitCode -eq 0 -or $process.ExitCode -eq 3010) {
            ...
            Write-Host "   ⚠️ Código: $($process.ExitCode)" -ForegroundColor Yellow
            return $false
        }
    } catch {
...
║      ╚═════╝ ╚══════╝ ╚═╝   ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝   ╚═╝   ╚══════╝
║                                                                  ║
║               RUNTIMES INSTALLER v3.0 - ONE LINE                 ║
║                                                                  ║
...
Write-Host "   📌 Instalação automática de todos os runtimes necessários" -ForegroundColor White
Write-Host "   📌 Para BlueStacks, jogos e aplicações em geral" -ForegroundColor White
Write-Host ""

$menuOptions = @(
    "⚡ INSTALAR TUDO (Recomendado - 1 clique)",
    "📦 Instalar Visual C++ 2005-2022 (AIO)",
    "🟢 Instalar Visual C++ 2005",
    "🟡 Instalar Visual C++ 2008",
    "🟠 Instalar Visual C++ 2010",
    "🔵 Instalar Visual C++ 2012",
    "🟣 Instalar Visual C++ 2013",
    "🔴 Instalar Visual C++ 2015-2022",
    "📁 Instalar DirectX 9.0c",
    "🌐 Instalar .NET 7.0 + 8.0 + ASP.NET",
    "🌐 Instalar .NET Framework 4.8.1",
...
do {
    ...
    for ($i = 0; $i -lt $menuOptions.Count; $i++) {
        if ($i -eq 0) { Write-Host "   [$($i+1)] $($menuOptions[$i])" -ForegroundColor Green }
        else { Write-Host "   [$($i+1)] $($menuOptions[$i])" }
    }
    ...
    switch ($choice) {
        "1" {
            ...
            Install-Runtime -Name "ASP.NET Core 8.0 Runtime" -Url "https://download.visualstudio.microsoft.com/download/pr/1fcedc74-2052-4768-8a5b-ca139b427d1e/42f48f1d36a37225e2c676201a31e15b/aspnetcore-runtime-8.0.12-win-x64.exe" -Args "/quiet /norestart" -FileName "aspnet-runtime.exe"
        ...
        "12" {
            Write-Host "`n🔍 VERIFICANDO COMPONENTES INSTALADOS..." -ForegroundColor Cyan
            Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Gray
            
            # Verificar VC++ Redists
            $vcPaths = @(
            ...
            foreach ($path in $vcPaths) {
                if (Test-Path $path) {
                    ...
                    if ($version) { Write-Host "   ✅ VC++ 2015-2022: $version" -ForegroundColor Green }
                }
            }
            ...
            if ($dotnet) {
                $version = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full" -Name "Release" -ErrorAction SilentlyContinue).Release
                Write-Host "   ✅ .NET Framework: Versão $version" -ForegroundColor Green
            }
            
            Write-Host "`n💡 Para verificação completa, use: 'Get-WmiObject Win32_Product'" -ForegroundColor Gray
        }
